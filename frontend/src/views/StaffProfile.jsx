      ...
      <header className="bg-white border-b sticky top-0 z-50 shadow-sm">
        <nav className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <Link to="/staff" className="text-gray-700 hover:text-green-600 transition">Staff Portal</Link>
            <Link to="/staff/profile" className="text-green-600 font-semibold">Profile</Link>
          </div>
        </nav>
      </header>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-800 mb-2">{profile?.name || 'Staff'}</h1>
            <p className="text-gray-600">{profile?.email}</p>
            {profile?.staffId && (
              <p className="text-sm text-gray-500 mt-1">Staff ID: {profile.staffId}</p>
            )}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-blue-50 rounded-lg p-6">
              ...
              <div className="text-sm text-gray-500 mt-2">Track your performance</div>
            </div>
          </div>
          ...
          <div className="border-t pt-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                {isEditing ? (
                  <input value={teamInput} onChange={e=>setTeamInput(e.target.value)} className="border rounded px-3 py-2 flex-1 max-w-xs" placeholder="Team Alpha" />
                ) : (
                  <span className="font-semibold">{profile?.teamName || 'Not assigned'}</span>
                )}
              </div>
              <div className="flex justify-between">
              ...
              {!isEditing ? (
                ...
                <>
                  <button onClick={async ()=>{
                    try {
                      await api.put('/profile', { name: nameInput, teamName: teamInput })
                      setProfile({ ...profile, name: nameInput, teamName: teamInput })
                      setIsEditing(false)
                    } catch (e) { alert('Failed to update profile') }
                  }} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Save</button>
                  <button onClick={()=>{ setIsEditing(false); setNameInput(profile?.name||''); setTeamInput(profile?.teamName || '') }} className="bg-red-100 text-red-700 px-4 py-2 rounded hover:bg-red-200">Cancel</button>
                </>
              )}
            </div>
          </div>
